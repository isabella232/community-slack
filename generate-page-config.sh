...
dev_url=undefined
if [ -e output-dev.json ] ; then
  dev_url="\"$(cat output-dev.json | jq -r .ServiceEndpoint)/invite\""
fi

prod_url=undefined
if [ -e output-prod.json ] ; then
  prod_url="\"$(cat output-prod.json | jq -r .ServiceEndpoint)/invite\""
fi
